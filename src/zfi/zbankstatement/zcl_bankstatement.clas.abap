  ...
  CLASS-METHODS :
      read_posts
        ...
        IMPORTING CompanyCode      TYPE string ##NEEDED
                  ...
                  ProfitCenter     TYPE string ##NEEDED
...
*     IngoinGL  = |{ IngoinGL ALPHA = IN }|.
*    OutgoinGL  = |{ IngoinGL ALPHA = IN }|.

    IF strlen( IngoinGL ) = 8.
      CONCATENATE '00' IngoinGL INTO IngoinGL.
    ELSEIF strlen( IngoinGL ) = 9.
      CONCATENATE '0' IngoinGL INTO IngoinGL.
    ENDIF.

    IF strlen( OutgoinGL ) = 8.
      CONCATENATE '00' OutgoinGL INTO OutgoinGL.
    ELSEIF strlen( OutgoinGL ) = 9.
      CONCATENATE '0' OutgoinGL INTO OutgoinGL.
    ENDIF.
    ...
    WHERE a~GLAccount IN ( @ingoingl, @outgoingl )
    INTO @DATA(gl_text).
...
*    gltext = | { gl_text-GLAccountLongName } [{ gl_text-GLAccount }]|.
    ...
    IF lt_accounting_documents IS NOT INITIAL.
      IF profitcenter IS NOT INITIAL.
        ...
        FIELDS
         ...
         AND ( a~GLAccount = @ingoingl AND a~GLAccount = @outgoingl )
          AND ( a~ProfitCenter = @ProfitCenter )
    ...
    SELECT FROM  i_accountingdocumentjournal( P_Language = 'E' )
        ...
        WHERE PostingDate LT @lastdate AND Ledger = '0L' AND IsReversed IS INITIAL
         ...
         AND GLAccount IN ( @ingoingl, @outgoingl )
