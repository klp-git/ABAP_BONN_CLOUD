...
CLASS LHC_RAP_TDAT_CTS IMPLEMENTATION.
  METHOD GET.
    result = mbc_cp_api=>rap_tdat_cts( tdat_name = 'ZTMGFORWITHHOLDINGTA'
                                       table_entity_relations = VALUE #(
                                         ( entity = 'TmgForWithHoldingTa' table = 'ZWITHHOLDINGTMG' )
                                       ) ) ##NO_TEXT.
  ENDMETHOD.
ENDCLASS.
CLASS LHC_ZI_TMGFORWITHHOLDINGTA_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
...
CLASS LHC_ZI_TMGFORWITHHOLDINGTA_S IMPLEMENTATION.
  ...
  METHOD SELECTCUSTOMIZINGTRANSPTREQ.
    MODIFY ENTITIES OF ZI_TmgForWithHoldingTa_S IN LOCAL MODE
    ...
    READ ENTITIES OF ZI_TmgForWithHoldingTa_S IN LOCAL MODE
      ENTITY TmgForWithHoldinAll
  ...
  METHOD GET_GLOBAL_AUTHORIZATIONS.
    AUTHORITY-CHECK OBJECT 'S_TABU_NAM' ID 'TABLE' FIELD 'ZI_TMGFORWITHHOLDINGTA' ID 'ACTVT' FIELD '02'.
  ...
  METHOD EDIT.
    ...
    IF transport_request IS NOT INITIAL.
      MODIFY ENTITY IN LOCAL MODE ZI_TmgForWithHoldingTa_S
  ...
  ENDMETHOD.
ENDCLASS.
CLASS LSC_ZI_TMGFORWITHHOLDINGTA_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_SAVER.
  PROTECTED SECTION.
    METHODS:
      SAVE_MODIFIED REDEFINITION.
ENDCLASS.

CLASS LSC_ZI_TMGFORWITHHOLDINGTA_S IMPLEMENTATION.
  METHOD SAVE_MODIFIED.
    ...
    IF transport_from_singleton IS NOT INITIAL.
      lhc_rap_tdat_cts=>get( )->record_changes(
                                  ...
                                  create            = REF #( create )
                                  update            = REF #( update )
                                  delete            = REF #( delete ) )->update_last_changed_date_time( view_entity_name   = 'ZI_TMGFORWITHHOLDINGTA'
                                                                                                        maintenance_object = 'ZTMGFORWITHHOLDINGTA' ).
    ENDIF.
  ENDMETHOD.
ENDCLASS.
CLASS LHC_ZI_TMGFORWITHHOLDINGTA DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
...
CLASS LHC_ZI_TMGFORWITHHOLDINGTA IMPLEMENTATION.
  ...
  METHOD COPYTMGFORWITHHOLDINGTA.
    DATA new_TmgForWithHoldingTa TYPE TABLE FOR CREATE ZI_TmgForWithHoldingTa_S\_TmgForWithHoldingTa.
  ...
  METHOD VALIDATETRANSPORTREQUEST.
    ...
    DATA change TYPE REQUEST FOR CHANGE ZI_TmgForWithHoldingTa_S.
    READ ENTITY IN LOCAL MODE ZI_TmgForWithHoldingTa_S
    ...
    lhc_rap_tdat_cts=>get( )->validate_all_changes(
                                ...
                                table_validation_keys = VALUE #(
                                                          ( table = 'ZWITHHOLDINGTMG' keys = REF #( keys_TmgForWithHoldingTa ) )
                                                               )
