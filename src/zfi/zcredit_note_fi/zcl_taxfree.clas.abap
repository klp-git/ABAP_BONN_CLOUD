CLASS zcl_taxfree DEFINITION
  ...
  PROTECTED SECTION.
  PRIVATE SECTION.
    CONSTANTS lc_template_name TYPE string VALUE 'zdebitnote/zdebitnote'.
ENDCLASS.



CLASS ZCL_TAXFREE IMPLEMENTATION.
  ...
  METHOD read_posts.
    ...
    SELECT SINGLE FROM I_SupplierInvoiceAPI01 AS a
       ...
       FIELDS a~CompanyCode , b~CompanyCodeName , h~gstin_no , d~SupplierName , d~Region, d~StreetName,d~CityName,d~PostalCode,
...
*             f~TaxAmountInCoCodeCrcy AS TaxAmountInCoCodeCrcy
    ...
    LOOP AT it_items INTO DATA(lv_item).
      CLEAR: igst, sgst, cgst, gst , lv_tax_amount,igst_amt,sgst_amt,cgst_amt, gst_amt.
