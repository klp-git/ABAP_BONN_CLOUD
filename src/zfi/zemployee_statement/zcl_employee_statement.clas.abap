CLASS ZCL_EMPLOYEE_STATEMENT DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_EMPLOYEE_STATEMENT IMPLEMENTATION.
...
*                  LEFT JOIN i_customer AS d ON a~Supplier = d~Supplier
                  WHERE a~supplier = @employee  INTO CORRESPONDING FIELDS OF @wa_add1.
...
*        AND a~Supplier = @employee
*        AND a~financialaccounttype = 'K'
*        AND a~specialglcode <> 'F'
*        And a~GLAccount = '0021517100'
...
*        AND a~Supplier = @employee
*        AND a~financialaccounttype = 'K'
*        AND a~specialglcode is not initial
*        and a~AmountInCompanyCodeCurrency > 0
*        AND ( a~ProfitCenter = @ProfitCenter OR @ProfitCenter = '' )
*        AND a~PostingDate GE @date3 AND a~PostingDate LE @date2
*        GROUP BY
*      a~billingdocument,
...
*        INTO table it_tab .
    ...
    " Second select (corrected FROM clause)
    SELECT
        a~billingdocument,
        a~accountingdocument,
        a~postingdate,
    ...
    APPEND LINES OF lt_tab3 TO it_tab.
    ...
    IF it_tab IS NOT INITIAL.
     ...
     WHERE a~companycode = @it_tab-companycode
       ...
       AND ( @profitcenter IS INITIAL OR a~profitcenter = @profitcenter )
       AND a~postingdate BETWEEN @date3 AND @date2
     INTO TABLE @DATA(it_tab4).

      APPEND LINES OF it_tab4 TO it_tab2.
...
LEFT OUTER  JOIN  I_Bank_2 AS f ON ( f~BankInternalID = b~Bank  )  WHERE a~Supplier = @employee  INTO @DATA(supplier).
