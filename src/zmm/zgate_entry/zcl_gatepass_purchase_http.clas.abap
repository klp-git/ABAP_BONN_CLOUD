class ZCL_GATEPASS_PURCHASE_HTTP definition
   public
  create public .
...
CLASS ZCL_GATEPASS_PURCHASE_HTTP IMPLEMENTATION.


METHOD get_html.    "Response HTML for GET request
    html = |<html> \n| &&
  |<body> \n| &&
  |<title>GatePass-Purchase</title> \n| &&
*  |<form action="{ url }" method="POST">\n| &&
  |<form action="/sap/bc/http/sap/ZGATEPASS_PURCHASE_HTTP?sap-client=080" method="POST">\n| &&
  |<H2>GatePass-Purchase</H2> \n| &&
...
*     req_uri = request->get_request_uri( ).
    ...
    req_uri = '/sap/bc/http/sap/ZGATEPASS_PURCHASE_HTTP?sap-client=080'.
...
*        TRANSLATE cc to UPPER CASE.
        ...
        IF lv_ge IS NOT INITIAL.
          ...
          TRY.
              DATA(pdf) = zcl_gatepass_purchase_driver=>read_posts( lv_gateentry = gee  ) .
