class ZCL_NRGP_HTTP definition
   public
  create public .
...
CLASS ZCL_NRGP_HTTP IMPLEMENTATION.
...
*  |<form action="{ url }" method="POST">\n| &&
  |<form action="/sap/bc/http/sap/ZNRGP_HTTP?sap-client=080" method="POST">\n| &&
  |<H2>NRGP</H2> \n| &&
...
*     req_uri = request->get_request_uri( ).
    ...
    req_uri = '/sap/bc/http/sap/ZNRGP_HTTP?sap-client=080'.
...
*        TRANSLATE cc to UPPER CASE.
        ...
        IF lv_ge IS NOT INITIAL.
          ...
          TRY.
              DATA(pdf) = znrgp_driver=>read_posts( lv_gateentry = gee  ) .
