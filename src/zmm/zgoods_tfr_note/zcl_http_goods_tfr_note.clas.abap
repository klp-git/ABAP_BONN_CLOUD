class ZCL_HTTP_GOODS_TFR_NOTE definition
   public
   create public .
public section.
...
CLASS ZCL_HTTP_GOODS_TFR_NOTE IMPLEMENTATION.
...
*  |<form action="{ url }" method="POST">\n| &&
  |<form action="/sap/bc/http/sap/ZHTTP_GOODS_TFR_NOTE?sap-client=080" method="POST">\n| &&
...
*     req_uri = request->get_request_uri( ).
    ...
    req_uri = '/sap/bc/http/sap/ZHTTP_GOODS_TFR_NOTE?sap-client=080'.
...
*        DATA(yr) = request->get_form_field( `lv_fiscalyear` ).
       ...
       DATA(pt) = request->get_form_field( `lv_plant` ).
...
*        into @data(lv_ge).
 SELECT SINGLE FROM I_MATERIALDOCUMENTITEM_2 WITH PRIVILEGED ACCESS
 FIELDS  MaterialDocument
 WHERE MaterialDocument = @gee AND CompanyCode = @cc AND Plant = @pt
 INTO @DATA(lv_ge).

        IF lv_ge IS NOT INITIAL.

          TRY.
              DATA(pdf) = zcl_goods_tfr_note=>read_posts( lv_gateentry = gee   lv_companycode = cc lv_plant = pt ) .
