...
CLASS lhc_ZIRN IMPLEMENTATION.
  ...
  METHOD createIRNData.
    ...
    LEFT JOIN i_customer AS b ON a~PayerParty = b~Customer
        ...
        a~Plant,
        a~PayerParty,
        b~CustomerName, c~DocumentReferenceID
        WHERE a~Plant = @plant
    ...
    LOOP AT lt INTO DATA(wa).
       ...
       i_billingdocumentitem AS a
        ...
        and a~BillingDocumentDate = @wa-BillingDocumentDate
    ...
    ENTITY zirn
    CREATE
    FIELDS ( Bukrs billingdocno billingdate documentreferenceid plant partycode partyname moduletype lastchangedat grossweight netweight )
    WITH VALUE #(
      ( %cid = 'cidap'
        Bukrs = wa-CompanyCode
        billingdocno = wa-BillingDocument
        billingdate = wa-BillingDocumentDate
        documentreferenceid = wa-DocumentReferenceID
        plant = wa-Plant
